...
public static class TaskExtensions
{
    ...
    {
        return task.ContinueWith(t => t.Result.AsReadOnly()).ConvertTask().ToTaskOf<IReadOnlyCollection<T>>();
    }
            ...
            where TResult : class
        {
            if (!typeof(TResult).IsAssignableFrom(typeof(T)))
                throw new InvalidOperationException(
                    $"{typeof(TResult).Name} is not assignable from {typeof(T).Name}");
